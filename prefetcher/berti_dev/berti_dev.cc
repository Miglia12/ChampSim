...
berti_dev::LatencyTable::LatencyTable(const int size_param) : size(size_param) { latencyt = new latency_table[size_param]; }

berti_dev::LatencyTable::~LatencyTable() { delete[] latencyt; }

uint8_t berti_dev::LatencyTable::add(champsim::block_number addr, uint64_t tag, bool pf, uint64_t cycle)
{
  /*
...
uint64_t berti_dev::LatencyTable::del(champsim::block_number addr)
{
  /*
...
uint64_t berti_dev::LatencyTable::get(champsim::block_number addr)
{
  /*
...
uint64_t berti_dev::LatencyTable::get_tag(champsim::block_number addr)
{
  /*
...
berti_dev::ShadowCache::ShadowCache(const int sets_param, const int ways_param)
{
...
berti_dev::ShadowCache::~ShadowCache()
{
...
bool berti_dev::ShadowCache::add(uint32_t set, uint32_t way, champsim::block_number addr, bool pf, uint64_t lat)
{
  /*
...
bool berti_dev::ShadowCache::get(champsim::block_number addr)
{
  /*
   * Parameters:
   *      - addr: cache block v_addr
...
void berti_dev::ShadowCache::set_pf(champsim::block_number addr, bool pf)
{
  /*
   * Parameters:
   *      - addr: cache block v_addr
...
bool berti_dev::ShadowCache::is_pf(champsim::block_number addr)
{
  /*
   * Parameters:
   *      - addr: cache block v_addr
...
uint64_t berti_dev::ShadowCache::get_latency(champsim::block_number addr)
{
  /*
   * Init shadow cache
   *
...
/******************************************************************************/
berti_dev::HistoryTable::HistoryTable()
{
...
void berti_dev::HistoryTable::add(uint64_t tag, champsim::block_number addr, uint64_t cycle)
{
  /*
...
uint16_t berti_dev::HistoryTable::get_aux(uint32_t latency, uint64_t tag, champsim::block_number act_addr, uint64_t* tags, champsim::block_number* addr,
                                          uint64_t cycle)
{
  /*
...
uint16_t berti_dev::HistoryTable::get(uint32_t latency, uint64_t tag, champsim::block_number act_addr, uint64_t* tags, champsim::block_number* addr,
                                      uint64_t cycle)
{
  /*
   * Return an array (by parameter) with all the possible PC that can launch
...
void berti_dev::prefetcher_cycle_operate()
{
  // Nothing to do here
}

uint32_t berti_dev::prefetcher_cache_operate(champsim::address addr, champsim::address ip, uint8_t cache_hit, bool useful_prefetch, access_type type,
...
uint32_t berti_dev::prefetcher_cache_fill(champsim::address addr, long set, long way, uint8_t prefetch, champsim::address evicted_addr, uint32_t metadata_in)
{
  champsim::block_number line_addr{addr};
  ...
  if constexpr (champsim::debug_print) {
    std::cout << "[BERTI] fill addr: " << line_addr;
...
void berti_dev::prefetcher_final_stats()
{
