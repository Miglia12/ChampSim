...
void spp_dram::prefetcher_initialize()
{
  std::cout << "Initialize SPP-DRAM (SPP with DRAM prefetching)" << std::endl;
...
void spp_dram::prefetcher_cycle_operate() {}

uint32_t spp_dram::prefetcher_cache_operate(champsim::address addr, champsim::address ip, uint8_t cache_hit, bool useful_prefetch, access_type type,
                                            uint32_t metadata_in)
{
  ...
  typename spp_dram::offset_type::difference_type delta = 0;
  std::vector<typename spp_dram::offset_type::difference_type> delta_q(intern_->MSHR_SIZE);
  ...
  FILTER.check(addr, spp_dram::L2C_DEMAND);
    ...
    for (uint32_t i = pf_q_head; i < pf_q_tail; i++) {
      if (confidence_q[i] >= PF_THRESHOLD) {
        ...
        if (champsim::page_number{pf_addr} == page) { // Prefetch request is in the same physical page
          if (FILTER.check(pf_addr, ((confidence_q[i] >= FILL_THRESHOLD) ? spp_dram::SPP_L2C_PREFETCH : spp_dram::SPP_DRAM_PREFETCH))) {
            // For lower confidence, prefetch to DRAM (fill_this_level=false, prefetch_at_llc=true)
            if (confidence_q[i] >= FILL_THRESHOLD) {
              prefetch_line(pf_addr, true, 0, false); // L2 prefetch 
              GHR.pf_issued++;
            } else {
              // The false for fill_this_level means it opens the DRAM row but doesn't store data
              prefetch_line(pf_addr, false, 0, true);
            }
            ...
            if constexpr (SPP_DEBUG_PRINT) {
              if (confidence_q[i] >= FILL_THRESHOLD) {
                ...
                std::cout << "[ChampSim] SPP DRAM prefetch issued (not counted in GHR)" << std::endl;
              }
            }
          ...
          if constexpr (GHR_ON) {
            ...
            GHR.update_entry(curr_sig, confidence_q[i], spp_dram::offset_type{pf_addr}, delta_q[i]);
          }
        }
...
uint32_t spp_dram::prefetcher_cache_fill(champsim::address addr, long set, long way, uint8_t prefetch, champsim::address evicted_addr, uint32_t metadata_in)
{
  if constexpr (FILTER_ON) {
    if constexpr (SPP_DEBUG_PRINT) {
      std::cout << std::endl;
    }
    FILTER.check(evicted_addr, spp_dram::L2C_EVICT);
  }

  return metadata_in;
}

void spp_dram::prefetcher_final_stats() {}

// TODO: Find a good 64-bit hash function
uint64_t spp_dram::get_hash(uint64_t key)
{
...
void spp_dram::SIGNATURE_TABLE::read_and_update_sig(champsim::address addr, uint32_t& last_sig, uint32_t& curr_sig,
...
void spp_dram::PATTERN_TABLE::update_pattern(uint32_t last_sig, typename offset_type::difference_type curr_delta)
{
...
void spp_dram::PATTERN_TABLE::read_pattern(uint32_t curr_sig, std::vector<typename offset_type::difference_type>& delta_q, std::vector<uint32_t>& confidence_q,
...
bool spp_dram::PREFETCH_FILTER::check(champsim::address check_addr, FILTER_REQUEST filter_request)
{
  ...
  case spp_dram::SPP_L2C_PREFETCH:
  ...
  case spp_dram::SPP_DRAM_PREFETCH:
    if ((valid[quotient] || useful[quotient]) && remainder_tag[quotient] == remainder) {
      if constexpr (SPP_DEBUG_PRINT) {
        std::cout << "[FILTER] " << __func__ << " line is already in the filter check_addr: " << check_addr << " cache_line: " << cache_line;
        std::cout << " quotient: " << quotient << " valid: " << valid[quotient] << " useful: " << useful[quotient] << std::endl;
      ...
      // For DRAM prefetches, we don't set the valid bit since we're not caching the data
...
void spp_dram::GLOBAL_REGISTER::update_entry(uint32_t pf_sig, uint32_t pf_confidence, offset_type pf_offset, typename offset_type::difference_type pf_delta)
{
...
uint32_t spp_dram::GLOBAL_REGISTER::check_entry(offset_type page_offset)
{
